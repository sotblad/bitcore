...
**The peer-to-peer networking protocol for BTX.**

`bitcore-p2p-btx` adds [Bitcore protocol](https://en.bitcoin.it/wiki/Protocol_documentation) support for Bitcore.
...
```sh
npm install bitcore-p2p-btx
```

In order to connect to the Bitcore BTX network, you'll need to know the IP address of at least one node of the network, or use [Pool](./docs/pool.md) to discover peers using a DNS seed.

```javascript
var Peer = require('bitcore-p2p-btx').Peer;
