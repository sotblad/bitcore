...
describe('Unit', function() {
  ...
  it('has property accesors "BTX", "mBTX", "uBTX", "bits", and "satoshis"', function() {
    var unit = new Unit(1.2, 'BTX');
    unit.BTX.should.equal(1.2);
    unit.mBTX.should.equal(1200);
    unit.uBTX.should.equal(1200000);
  ...
  it('a string amount is allowed', function() {
    ...
    unit = Unit.fromBTX('1.00001');
    unit.BTX.should.equal(1.00001);

    unit = Unit.fromMilis('1.00001');
    unit.mBTX.should.equal(1.00001);
    ...
    unit.BTX.should.equal(0.12285714);
  });
  ...
  it('exposes unit codes', function() {
    should.exist(Unit.BTX);
    Unit.BTX.should.equal('BTX');

    should.exist(Unit.mBTX);
    Unit.mBTX.should.equal('mBTX');
  ...
  it('exposes a method that converts to different units', function() {
    var unit = new Unit(1.3, 'BTX');
    unit.to(Unit.BTX).should.equal(unit.BTX);
    unit.to(Unit.mBTX).should.equal(unit.mBTX);
  ...
  it('exposes shorthand conversion methods', function() {
    var unit = new Unit(1.3, 'BTX');
    unit.toBTX().should.equal(unit.BTX);
    unit.toMilis().should.equal(unit.mBTX);
    unit.toMillis().should.equal(unit.mBTX);
  ...
  it('can be imported and exported from/to JSON', function() {
    var json = JSON.stringify({amount:1.3, code:'BTX'});
  ...
  it('fails when the unit is not recognized', function() {
    ...
    expect(function() {
      return new Unit(100, 'BTX').to('USD');
    }).to.throw(errors.Unit.UnknownCode);
  });
  ...
  it('fails when the exchange rate is invalid', function() {
    ...
    expect(function() {
      return new Unit(100, 'BTX').atRate(-123);
    }).to.throw(errors.Unit.InvalidRate);
  });
