...
Note also that an output can be used only once. That's why there's a concept of "change address" in the bitcoin ecosystem: if an output of 10 BTX is available for me to spend, but I only need to transmit 1 BTX, I'll create a transaction with two outputs, one with 1 BTX that I want to spend, and the other with 9 BTX to a change address, so I can spend this 9 BTX with another private key that I own.
...
Now, this could just be serialized to hexadecimal ASCII values (`transaction.serialize()`) and sent over to the bitcored reference client.
...
var transaction = new Transaction().fee(1e8);  // Generous fee of 1 BTX
```
