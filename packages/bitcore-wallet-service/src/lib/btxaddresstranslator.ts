...
const Bitcore_ = {
  ...
  bch: require('bitcore-lib-btx')
};

export class BTXAddressTranslator {
  static getAddressCoin(address) {
    try {
      ...
      try {
        const a = new Bitcore_['btx'].Address(address);
  ...
  static translate(addresses, to, from?) {
    ...
    from = from || BTXAddressTranslator.getAddressCoin(addresses[0]);
    ...
    if (from == to) {
      ...
      ret = _.filter(
        _.map(addresses, (x) => {
          const bitcore = Bitcore_[from == 'legacy' ? 'btc' : 'btx'];
          let orig;
          ...
          if (to == 'cashaddr') {
            ...
            return Bitcore_['btx'].Address.fromObject(orig).toLegacyAddress();
          } else if (to == 'legacy') {
...
module.exports = BTXAddressTranslator;
